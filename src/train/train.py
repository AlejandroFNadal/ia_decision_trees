...
def train(df, class_name,method) -> Node:
    ...
    if method == 'gain':
        decisionTree(set_init, set_init.get_attributes(),root_node,0,classValues,False)
    else:
        decisionTree(set_init, set_init.get_attributes(),root_node,0,classValues,True)
    return root_node